...
temp_data=data(ib~=0,:);%现有数据
new_cls_data=data(ib==0,:);%新的微集群数据
temp_lb=block_labels(ib~=0);%现有标签
new_cls_lbs=block_labels(ib==0);%新的微集群标签
new_label=[];
if size(new_cls_lbs,1)>1
    new_label=new_cls_lbs(1);%新标签
end
for i=1:size(data,1)
    if ismember(block_labels(i),tr_cls_lb)
        ...
        if tr_cls_lb(p_label)==block_labels(i)
            ...
            Model(idx,6)=num2cell(cell2mat(Model(idx,6))+1);%新类加入模型
...
[ Model ] = break_MC( Model,CurrentTime );%删除旧的集群和低置信度集群
...
for i=1:size(temp_data,1)
    ex.data=temp_data(i,:);
    ex.lb=temp_lb(i);
    clu_centers=cell2mat(Model(:,7));%集群中心
    [idx, D]=knnsearch(clu_centers,ex.data,'NSMethod','exhaustive');
    r=Model{idx,9};
    if D<=r
        
        [ Model ] = update_micro(Model,ex,idx, tr_cls_lb, CurrentTime);%更新集群
