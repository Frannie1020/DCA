function [ Model, ShortMem, det_new] = detection_novel_class(Model,ShortMem,tr_cls, block_lb)
    global num_cluster;
    global block_size;
    K=ceil(num_cluster*size(ShortMem,1)/block_size);%K个初始微集群
    num_replicates=1;
    class_center=cell2mat(Model(:,8));%类中心
    M_Clu_C=cell2mat(Model(:,7));%微集群
    ShortMem_data=cell2mat(ShortMem(:,1));
    cls_dim=size(ShortMem_data,2);
    sh_lb=cell2mat(ShortMem(:,2));
    sh_lb=sh_lb(:,1);
    Total_new=~ismember(block_lb,tr_cls);%新类总数
    DimSz=size(Model{1,1},2);


    global lofT; global nooftime; global lofk;
    ...
    global totaltime;%总时间
    lof_data={};
    cov_lof_data={};
    tic
    for i=1: size(tr_cls,1)
        lof_data_temp=[];
        cls_data = cellfun(@(x) x(i,:),Model(:,15),'UniformOutput',false);%集群数据
        cls_num = cellfun(@(y) y(i),Model(:,4),'UniformOutput',false);%集群数量
        ...
        for m=1:clsDsz
            ...
            if cls_num(m)<=2
                ...
                numofsample=ceil((totalex*cls_num(m))/totalsum);%取样数量
                
                if size(cls_data{m}{1,1},2)>1
                    mu=cls_data{m}{1,1};
                    sigma=cls_data{m}{1,2};

                    X_data= mvnrnd(mu,sigma,numofsample);%多元正态分布随机数
                end
            end
            %         if size(X_data,2)
            %             disp(X_data);
    ...
    totaltime=totaltime+time2;
      total_exist_ins=total_exist_ins+(block_size-sum(Total_new));%现有实例总数
   
    total_novel_ins=total_novel_ins+sum(Total_new);%新类实例总数
